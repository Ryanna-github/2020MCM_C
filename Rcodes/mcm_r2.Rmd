...
pc <- read.csv('../Data/pacifier.csv', stringsAsFactors = F)
head(pc)
```
...
```{r}
reviews <- pc$review_body
class(reviews)
length(reviews)
write.table(reviews, '../Data/pacifier_reviews.txt', col.names = FALSE, quote = FALSE, row.names = FALSE)
```
...
```{r}
setwd("D:/Ryanna/mcm_real/Rcodes")
pc_st <- read.csv('../Data/pacifier_senti.csv', stringsAsFactors = F)[,2]
# 拼接
names(pc_st)
pc$pos_score <- pc_st
# write.csv(pc, '../Data/pacifier_withsenti.csv')
```
...
# library(data.table)
# library(plyr)
pc <- pc[!is.na(pc$pos_score),]
na.omit(pc)
pc$total_votes <- as.numeric(pc$total_votes)
pc$helpful_votes <- as.numeric(pc$helpful_votes)
...
```{r}
great_idx = grepl("great", tolower(pc$review_body))
na999_idx = (pc$pos_score == 999)
change_idx = great_idx|na999_idx
m = median(pc$pos_score)
pc$pos_score[change_idx] = m + rnorm(1, 0, 0.3)
# ggplot(pc, aes(x = pos_score)) + geom_histogram()
# summary(pc$pos_score)
```

- 进一步数据筛选，情感得分失败的样本舍弃，并且生成新的关于有效性的贝叶斯指标

```{r}
pc <- pc[!is.na(pc$helpful_votes),]
averageratio<-sum(pc$helpful_votes)/sum(pc$total_votes)
pc$bys_weight<-(100*averageratio+pc$helpful_votes)/(100+pc$total_votes)
pc$bys_pos_score <- pc$bys_weight * pc$pos_score
pc$bys_pos_score <- as.numeric(pc$bys_pos_score)
```
...
pc_weight <- apply(pc[1:10,][,c("bys_pos_score", "star_rating")],2,min.max.norm)
dataframe <- apply(pc_weight,2,first1)
...
```{r}
pc$score <- w['bys_pos_score']*pc$bys_pos_score + w['star_rating']*pc$star_rating
```
...
```{r}
pc$review_date <- as.Date(pc$review_date, "%m/%d/%Y")
```
...
```{r}
setwd("D:/Ryanna/mcm_real/Rcodes")
write.csv(pc, '../Data/pacifier_withsenti.csv', row.names = F)
```
...
# 按照评分划分
# midscore <- 2.2
# pc_good_reviews <- pc[pc$score >= midscore, 'review_body']
# pc_bad_reviews <- pc[pc$score < midscore, 'review_body']
```


```{r}
# 按照时间划分
good_start <- as.Date('2007/11/1')
good_end <- as.Date('2007/12/31')
bad_start <- as.Date('2012/2/1')
bad_end <- as.Date('2012/3/31')

pc_good_reviews <- pc[pc$review_date >= good_start & pc$review_date <= good_end, 'review_body']
pc_bad_reviews <- pc[pc$review_date >= bad_start & pc$review_date <= bad_end, 'review_body']
```
...
```{r}
setwd("D:/Ryanna/mcm_real/Rcodes")
write.table(pc_good_reviews, '../Data/pacifier_reviews_good.txt', col.names = FALSE, quote = FALSE, row.names = FALSE)
write.table(pc_bad_reviews, '../Data/pacifier_reviews_bad.txt', col.names = FALSE, quote = FALSE, row.names = FALSE)
```
...
```{r}
setwd("D:/Ryanna/mcm_real/Rcodes")
pc_good_ti <- read.csv('../Data/pacifier_tfidf_good.csv', stringsAsFactors = F)
pc_bad_ti <- read.csv('../Data/pacifier_tfidf_bad.csv', stringsAsFactors = F)
```
...
# 与之前条数不同，需要重新写出
setwd("D:/Ryanna/mcm_real/Rcodes")
write.table(pc$review_body, '../Data/pacifier_reviews_less.txt', col.names = FALSE, quote = FALSE, row.names = FALSE)
```


```{r}
setwd("D:/Ryanna/mcm_real/Rcodes")
pc_tags <- read.csv('../Data/pacifier_tags.csv', stringsAsFactors = F)
pc$adj_num <- pc_tags$adj_num
pc$vb_num <- pc_tags$vb_num
pc$n_num <- pc_tags$n_num
```
